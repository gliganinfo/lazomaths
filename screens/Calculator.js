...
export default function Calculator(props) {
  ...
  const onEqualsPress = async() => {
    calc.equalsPressed()
    const mainDisplay = calc.getMainDisplay()
    
    // Show result and save Ans and PreAns
    ...
    // Save result to a persistent storage source
    const now = new Date()
    const jsonData = JSON.stringify({
      datetime: now.toString(),
      result: mainDisplay
    })
    await AsyncStorage.setItem(`@${nanoid()}`, jsonData)
      ...
      <View style={styles.mainContainer}>
        ...
        <View style={styles.buttonKeyboard}>
          ...
          <View style={styles.buttonRow}>
            {/* <CalcBtn text="(" bg="#E8E4BB" color="#97947C" onPress={() => onUnaryOperatorPress(oc.OpenParenOperator)} /> */}
            {/* <CalcBtn text=")" bg="#E8E4BB" color="#97947C" onPress={() => onUnaryOperatorPress(oc.CloseParenOperator)} /> */}
            <CalcBtn text="Ver historial" bg="#F2EECB" color="#97947C" flexType="2" onPress={() => props.navigation.navigate('History')} />
